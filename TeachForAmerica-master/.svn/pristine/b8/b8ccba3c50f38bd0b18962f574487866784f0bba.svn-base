...
public class BalanceCorpsYearsScoringCriteria implements
	...
	public double scoreCorpsMemberToCohort(CorpsMember cm, Cohort cohort) {
		...
		for (CohortDetail cohortDetail : cohort.getCohortDetails()) {
			...
			if (cohortDetail != null && cohortDetail.getCorpMember() != null && cohortDetail.getCorpMember().getCorpsYear() != null && TFAConstants.CORPS_YEAR1 <= cohortDetail.getCorpMember().getCorpsYear() 
					) {
				firstYearCount++;
			} else if (cohortDetail != null && cohortDetail.getCorpMember() != null && cohortDetail.getCorpMember().getCorpsYear() != null && TFAConstants.CORPS_YEAR2 <= cohortDetail.getCorpMember().getCorpsYear() 
					) {
				secondYearCount++;
			}
		
	}
		if (cm.getCorpsYear() != null && TFAConstants.CORPS_YEAR1  <= cm.getCorpsYear() ) {
