...
public class CriteriaServiceMockTest {
	...
	public void testGetCriteriaList() throws Exception {
		...
		/*HashMap<String, List<CriteriaBean>> criteriaMap = criteriaService
		 ...
		 CriteriaFormBean criteriaFormBean= criteriaService.getCriteriaFormBean();

		 
		 Assert.assertNotNull(criteriaFormBean);
			Assert.assertTrue(criteriaFormBean.getCriteriaCategoryBeanList().size() == 4); // Criteria Categories count should be 4.
		 
			Assert.assertTrue(criteriaFormBean.getCriteriaCategoryBeanList().get(0).getCriteriaBeans().size()==2);
			Assert.assertTrue(criteriaFormBean.getCriteriaCategoryBeanList().get(1).getCriteriaBeans().size()==1);
			Assert.assertTrue(criteriaFormBean.getCriteriaCategoryBeanList().get(2).getCriteriaBeans().size()==1);
			Assert.assertTrue(criteriaFormBean.getCriteriaCategoryBeanList().get(3).getCriteriaBeans().size()==1);
			//Assert.assertTrue(criteriaFormBean.getCateoryBeanList().);
