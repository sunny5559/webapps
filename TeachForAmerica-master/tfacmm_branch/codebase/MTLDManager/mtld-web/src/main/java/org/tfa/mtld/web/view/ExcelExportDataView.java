...
public class ExcelExportDataView extends AbstractExcelView{
	...
	protected void buildExcelDocument(Map<String, Object> model,
		...
		CriteriaFormBean criteriaFormBean = (CriteriaFormBean) dataMap.get("criteriaFormBean");
		...
		setCMHeader(cmHeaderRow,getCrieriaList(criteriaFormBean));
		...
		setCMRowsData(cmSheet , cohorts, regionCode,criteriaFormBean);
		setMTLDRowsData(mtldSheet, cohorts);
		
		//Setting data for criteria values
		setCriteriaRowsData(criteriaSheet,criteriaFormBean);
	}
	...
	private void setCMRowsData(HSSFSheet cmSheet, List<CohortBean> cohorts, String regionCode, CriteriaFormBean criteriaFormBean) {
		int rowNum =1 ;
		for (Iterator iterator = cohorts.iterator(); iterator.hasNext();) {
			CohortBean cohort = (CohortBean) iterator.next();
			...
			for (Iterator iterator2 = cohortDetails.iterator(); iterator2
				...
				if(corpsMember != null){
					...
					hssfRow.createCell(cellNum++).setCellValue(corpsMember.getPersonOfColor()?"Y":"N");
					...
					List<CriteriaBean> criteriaBeanList= getCrieriaList(criteriaFormBean);
	...
	private void setCriteriaRowsData(HSSFSheet criteriaSheet,
		...
		for (CriteriaCategoryBean categoryBean : criteriaFormBean.getCriteriaCategoryBeanList()) {

			for (CriteriaBean bean : categoryBean.getCriteriaBeans()) {
	...
	private List<CriteriaBean> getCrieriaList(CriteriaFormBean criteriaFormBean){
		List<CriteriaBean> beanList=new ArrayList<CriteriaBean>();
		
	 	 for(CriteriaCategoryBean categoryBean:  criteriaFormBean.getCriteriaCategoryBeanList()){
	 		beanList.addAll(categoryBean.getCriteriaBeans());
	 		 
	 	 }
		
	/*	for (Map.Entry<String, List<CriteriaBean>> entry : criteriaFormBean
				.entrySet()) {
